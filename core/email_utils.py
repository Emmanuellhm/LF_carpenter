...
class LFEmailService:
    ...
    def send_welcome_email(self, user):
        ...
        context = {
            'user': user,
            'title': f'¡Bienvenido a {self.site_name}!',
            'message': f'Hola {user.full_name}, gracias por registrarte en nuestra plataforma.',
        }
        return self._send_email(
            to_email=user.email,
            subject='¡Bienvenido a LF Carpinter!',
    ...
    def send_project_status_email(self, solicitud, new_status):
        """
        Envía email cuando cambia el estado de una solicitud de proyecto.
        new_status: 'accepted', 'rejected', 'completed'
        """
        status_messages = {
            'accepted': 'tu solicitud de proyecto ha sido aceptada',
            'rejected': 'tu solicitud de proyecto ha sido rechazada',
            'completed': 'tu proyecto ha sido marcado como completado',
        }

        context = {
            'user': solicitud.user,
            'solicitud': solicitud,
            'carpenter': solicitud.carpenter,
            'status': new_status,
            'status_message': status_messages.get(new_status, 'ha habido una actualización'),
            'title': 'Actualización de tu proyecto',
        }
        return self._send_email(
            to_email=solicitud.user.email,
            subject=f'Actualización de tu proyecto - LF Carpinter',
