...
def registro_exitoso_usuario(request):
    """Página de registro exitoso para usuarios normales.
    No requiere autenticación; muestra un mensaje de confirmación.
    """
    return render(request, 'core/registro_exitoso.html')

def registro_exitoso_google(request):
    """Página de registro exitoso para usuarios que se registraron vía Google."""
    return render(request, 'core/registro_exitoso_google.html')


def explorar_carpinteros(request):
    """Vista placeholder para explorar carpinteros públicos."""
    # En futuro, se listarán objetos Carpintero aprobados.
    return render(request, 'core/explorar_carpinteros.html')


def perfil_publico_carpintero(request, carpintero_id):
    """Perfil público de un carpintero. Parámetro: carpintero_id (int)."""
    # Placeholder: en futuro se obtendrá el objeto Carpintero.
    return render(request, 'core/perfil_publico_carpintero.html', {'carpintero_id': carpintero_id})


def comprar_proyecto(request, portafolio_id):
    """Página placeholder para comprar un proyecto/portafolio."""
    # Placeholder: lógica de compra y pago será implementada después.
    return render(request, 'core/comprar_proyecto.html', {'portafolio_id': portafolio_id})


def completar_registro_social(request):
    """Vista placeholder para completar registro vía redes sociales."""
    return render(request, 'core/completar_social.html')


def marcar_notificacion_leida(request, notif_id):
    """Marca una notificación como leída (placeholder)."""
    # En producción, actualizaríamos el modelo Notificacion.
    from django.http import JsonResponse
    return JsonResponse({'status': 'ok', 'notif_id': notif_id})