...
<aside class="w-64 bg-white shadow-xl flex flex-col justify-between border-r border-stone-200">
  <div>
    <div class="flex flex-col items-center py-8 border-b border-stone-200">
      <!-- Foto de perfil -->
      <img src="../assets/img/fotoP.jpg" alt="Foto Perfil"
  ...
  <div class="flex-1 flex flex-col">
    <header class="flex items-center justify-between bg-white border-b border-stone-200 px-6 h-20 shadow-sm">
      ...
      <nav>
        <a href="#" class="font-semibold text-stone-700 hover:text-amber-600">Contáctanos</a>
      </nav>
    </header>
    ...
    <main class="p-10">
      ...
      <div id="historial-lista" class="space-y-4">
        <!-- Carpinteros contratados se agregarán aquí -->
      </div>
    </main>
  </div>
  ...
  <script>
    ...
    // ✅ Datos de ejemplo de carpinteros contratados (puedes reemplazarlo por datos reales)
    const historial = [
      {
      ...
      {
        nombre: "Andrés López",
        proyecto: "Silla ergonómica",
        fecha: "05/09/2025",
        estado: "Completado ✅"
      },
      {
        nombre: "Laura Martínez",
        proyecto: "Restauración de escritorio antiguo",
        fecha: "01/09/2025",
        estado: "Completado ✅"
      }
    ];

    // ✅ Mostrar historial dinámicamente
    const lista = document.getElementById("historial-lista");
    historial.forEach(item => {
      const div = document.createElement("div");
      div.className = "bg-white p-6 rounded-xl shadow-lg border border-stone-200";
      div.innerHTML = `
        <p class="font-semibold text-stone-800">🔧 Carpintero: ${item.nombre}</p>
        <p class="text-stone-600 text-sm">Proyecto: ${item.proyecto}</p>
        <p class="text-stone-600 text-sm">Fecha de contratación: ${item.fecha}</p>
