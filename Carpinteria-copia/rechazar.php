...
$id     = intval($_POST['id']);
$motivo = trim($_POST['motivo'] ?? 'No se especificó un motivo.');
...
if ($stmt->execute()) {
    ...
    // ── Correo de rechazo al carpintero ────────────────────────
    if ($row) {
        ...
        $cuerpo = "
            ...
            <p>Hola <strong>$nombre</strong>, lamentamos informarte que tu solicitud para unirte como carpintero en <strong>LF Carpintería</strong> no ha sido aprobada en esta ocasión.</p>
        ...
        enviarCorreo($row['email'], '❌ Tu solicitud no fue aprobada - LF Carpintería', $cuerpo);
    }
