  ...
  <div id="mensaje-exito" class="fixed top-4 right-4 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg z-50">
    <?php
      if ($_GET['success'] === 'solicitud_enviada') {
        echo '✅ Solicitud de proyecto enviada correctamente';
      }
    ?>
  </div>
  <script>
  ...
  <div id="mensaje-error" class="fixed top-4 right-4 bg-red-600 text-white px-6 py-3 rounded-lg shadow-lg z-50">
    <?php
      $error_msg = match($_GET['error']) {
        'datos_incompletos' => '❌ Por favor completa todos los campos requeridos',
        'carpintero_no_existe' => '❌ El carpintero seleccionado no existe',
        'carpintero_sin_usuario' => '❌ Error al procesar la solicitud',
        'error_servidor' => '❌ Error del servidor, intenta nuevamente',
        'error_al_enviar' => '❌ Error al enviar la solicitud',
          ...
          <?php foreach ($carpenters as $c): ?>
            <div class="bg-white border border-stone-200 rounded-xl p-6 shadow-sm hover:shadow-lg transition">
              ...
              <div class="space-y-2 mb-4">
                ...
                <?php
                  // Determinar ciudad para mostrar en tarjeta
                  $card_city = $c['user_city'] ?? '';
                  if (empty($card_city) && !empty($c['description'])) {
                    if (preg_match('/Ciudad:\s*([^|]+)/i', $c['description'], $mc)) {
                      $card_city = trim($mc[1]);
                    }
                  }
                ?>
                ...
                <div class="bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                  <p class="text-xs font-semibold text-stone-500 uppercase">Ubicación</p>
                  <p class="text-stone-800 font-medium flex items-center gap-1">
                    <i class="fas fa-map-marker-alt text-green-600 text-xs"></i>
                    <?php echo htmlspecialchars($card_city); ?>
                  </p>
                </div>
                <?php endif; ?>
              ...
              <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 p-8" onclick="event.stopPropagation()">
                ...
                <div class="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div class="space-y-2 text-sm">
                    ...
                    <p class="flex items-center gap-1"><strong>Ubicación:</strong> 
                      <?php if (!empty($city)): ?>
                        <i class="fas fa-map-marker-alt text-green-600 text-xs ml-1"></i>
                ...
                <div class="flex justify-center gap-3">
                  <?php if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] === 'user'): ?>
                    <button onclick="openRequestModal(<?php echo $c['user_id']; ?>, '<?php echo htmlspecialchars($c['carpenter_name'], ENT_QUOTES); ?>')" 
  ...
  <div id="modal-request-project" class="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center hidden z-50 backdrop-blur-sm" onclick="closeRequestModal()">
    <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
      ...
      <form id="request-project-form" action="solicitar_proyecto.php" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
        ...
        <div>
          <label class="block font-semibold text-stone-700 mb-2">Título del Proyecto *</label>
          <input type="text" name="title" required placeholder="Ej: Mesa de comedor rústica"
                 class="w-full px-4 py-2 border border-stone-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none">
        </div>

        <div>
          <label class="block font-semibold text-stone-700 mb-2">Descripción Detallada *</label>
