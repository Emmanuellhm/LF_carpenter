...
// Cargar datos actuales del usuario desde la tabla users (email, phone, city)
$stmtU = $conn->prepare("SELECT email, phone, city FROM users WHERE user_id = ? LIMIT 1");
$stmtU->bind_param('i', $user_id);
$stmtU->execute();
$resU = $stmtU->get_result();
if ($resU && $resU->num_rows > 0) {
    $rowU = $resU->fetch_assoc();
    $user['email'] = $rowU['email'] ?? '';
    $user['phone'] = $rowU['phone'] ?? '';
    $user['city'] = $rowU['city'] ?? '';
}
$stmtU->close();
...
// Priorizar ciudad de la tabla users sobre la del campo description
if (!empty($user['city'])) {
  $reg_city = $user['city'];
}
if (!empty($user['email'])) {
  ...
  <section id="dashboard" class="seccion">
    ...
    <div class="mt-10 bg-white p-8 rounded-xl shadow-lg border border-stone-200">
      ...
      <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="bg-stone-50 p-4 rounded-lg border border-stone-100">
          <p class="text-xs font-bold text-stone-400 uppercase mb-1">Ubicación</p>
          <p class="text-stone-800 font-medium flex items-center gap-1">
            <?php if (!empty($reg_city)): ?>
              <i class="fas fa-map-marker-alt text-amber-600 text-xs"></i>
              <?php echo htmlspecialchars($reg_city); ?>
            <?php else: ?>
              <span class="text-stone-400">No especificada</span>
            <?php endif; ?>
          </p>
        </div>
        <div class="bg-stone-50 p-4 rounded-lg border border-stone-100">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div class="bg-white p-8 rounded-xl shadow-lg border border-stone-200 h-fit">
        ...
        <div class="bg-stone-50 p-6 rounded-lg border border-stone-100 space-y-4">
          ...
          <div>
            <span class="block text-xs font-bold text-stone-400 uppercase tracking-wide">Ubicación</span>
            <span class="text-stone-800 font-medium flex items-center gap-1">
              <?php if (!empty($reg_city)): ?>
                <i class="fas fa-map-marker-alt text-amber-600 text-xs"></i>
                <?php echo htmlspecialchars($reg_city); ?>
              <?php else: ?>
...
function verDetalleProyecto(requestId) {
  ...
  // Descripción
  document.getElementById('proy-descripcion').innerHTML = req.project_description.replace(/\n/g, '<br>');
